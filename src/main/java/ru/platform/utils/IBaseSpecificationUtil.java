...
public interface IBaseSpecificationUtil<T, U>  {
    ...
    default Specification<T> gameFilter(String gameId,  String field){
        return (root, query, criteriaBuilder) -> {
            Join<OfferEntity, GameEntity> gameJoin = root.join(field);
            return criteriaBuilder.equal(gameJoin.get("id"), UUID.fromString(gameId));
        };
    }
